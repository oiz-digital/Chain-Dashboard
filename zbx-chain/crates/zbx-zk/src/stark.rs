...
impl StarkVerifier {
    ...
    fn verify_ood_consistency(
        ...
        let _z = challenges.first().copied().unwrap_or(Fp::one());

        // Verify: constraint_poly(z) matches composition of trace_poly evaluations at z
        // Production: full AIR constraint check here
        // Here: length consistency check
        if proof.ood_trace_values.len() < self.config.num_registers as usize {
            return Err(StarkError::InvalidProofStructure(
                "OOD values count < num_registers".into()
            ));
        }
