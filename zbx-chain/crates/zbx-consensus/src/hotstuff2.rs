...
impl TcAccumulator {
    ...
    /// re-check it.
    fn build_tc(&self) -> TimeoutCertificate {
    ...
    pub fn check_timer(&mut self, current_round: u64) -> Option<Hs2Event> {
        if self.round_timer.is_expired() {
            ...
            let share = TimeoutShare {
                ...
                // SEC-2026-05-09 (Pass-5): zero placeholder; the key
                // manager fills in the real BLS signature over
                // `signing_hash()` before broadcast (mirrors the Vote
                // path's VoteRequest delegation).
