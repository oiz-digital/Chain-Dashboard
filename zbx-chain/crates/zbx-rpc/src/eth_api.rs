...
pub fn dispatch_eth(method: &str, params: &Value, state: &RpcState) -> Result<Value, RpcError> {
    ...
    match method {
        ...
        "eth_maxPriorityFeePerGas"  => Ok(json!("0x3B9ACA00")),  // 1 gwei priority tip
...
fn gas_price_oracle(state: &RpcState) -> Result<Value, RpcError> {
    ...
    for h in from..=latest {
        if let Ok(Some(block)) = state.db.get_block_by_number(h) {
            base_fees.push(block.header.base_fee_per_gas);
        }
    }

    let price = if base_fees.is_empty() {
        MIN_PRICE
    } else {
        base_fees.sort_unstable();
        let median = base_fees[base_fees.len() / 2];
        // recommended = median × 1.1 + 1 gwei priority tip, floor at 1 gwei
        median
            .saturating_add(median / 10)           // +10% buffer
            .saturating_add(MIN_PRICE)             // +1 gwei tip
            .max(MIN_PRICE)
    };
...
fn eth_fee_history(params: &Value, state: &RpcState) -> Result<Value, RpcError> {
    ...
    let oldest = head.saturating_sub(count.saturating_sub(1));
    Ok(json!({
        "oldestBlock":   format!("0x{:x}", oldest),
        "baseFeePerGas": vec!["0x3B9ACA00"; (count + 1) as usize],
        "gasUsedRatio":  vec![0.5; count as usize],
        "reward":        vec![vec!["0x3B9ACA00"]; count as usize],
    }))
}
