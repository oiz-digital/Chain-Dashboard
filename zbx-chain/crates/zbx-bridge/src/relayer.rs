...
mod tests {
    ...
    /// TODO(crypto): once zbx_crypto exposes test-key helpers, wire up a full
    /// confirm() → execute() flow and verify:
    /// - Valid sig is accepted and appended to confirmations.
    /// - Invalid sig is rejected with no state change (MS1 fix).
    /// - verify_and_consume prevents execution replay (OUT1 fix).
    #[test]
    #[ignore = "requires ECDSA test keys — enable once zbx_crypto::test_keys is available"]
    fn ms1_invalid_sig_does_not_poison_confirmations() {}

    #[test]
    #[ignore = "requires ECDSA test keys — enable once zbx_crypto::test_keys is available"]
    fn out1_verify_and_consume_prevents_replay() {}
