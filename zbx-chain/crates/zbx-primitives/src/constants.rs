...
impl ReceiptStore {
    ...
    /// Store a receipt for a transaction.
    pub fn put(&self, tx_hash: [u8; 32], receipt: &StoredReceipt) -> Result<(), DbError> {
        let _ = (tx_hash, receipt);
        Ok(()) // Production: db.put(CF, tx_hash, rlp_encode(receipt))
    }

    /// Get a receipt by transaction hash.
    pub fn get(&self, tx_hash: [u8; 32]) -> Result<Option<StoredReceipt>, DbError> {
        let _ = tx_hash;
        Ok(None) // Production: db.get(CF, tx_hash).map(rlp_decode)
    }

    /// Get all receipts in a block (by block hash prefix scan).
