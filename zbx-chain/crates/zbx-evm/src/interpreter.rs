...
impl<'a> EVMInterpreter<'a> {
    ...
    fn step(&mut self) -> Result<Option<ExitStatus>, EvmError> {
        ...
        match byte {
            ...
            0x32 => { // ORIGIN — tx origin; approximate with caller (no tx ctx)
                let mut w = [0u8; 32];
            ...
            0x3a => { // GASPRICE — return base_fee as approximation
            ...
            0x40 => { // BLOCKHASH — simplified: return 0 (no block history stored)
            ...
            0x44 => { // PREVRANDAO (formerly DIFFICULTY post-Merge)
                // Use block_number as entropy source (no randao in ctx yet).
                self.stack.push(u256_from_u64(self.ctx.block_number.wrapping_mul(0x517cc1b727220a95)))?;
            }
            0x45 => { // GASLIMIT — return a sensible fixed cap
                self.stack.push(u256_from_u64(30_000_000))?;
            }
            0x46 => { // CHAINID
            ...
            0x55 => { // SSTORE — static guard + simplified EIP-2200 gas
                ...
                // Simplified EIP-2200 gas:
                // new value == current → NOOP (100 gas, already paid as static)
                // current == 0, new != 0 → 20000
                // otherwise → 2900
                if current != value {
                    if u256_is_zero(&current) && !u256_is_zero(&value) {
                        self.consume_gas(GAS_SSTORE_SET)?;
                    } else {
                        self.consume_gas(GAS_SSTORE_RESET)?;
                    }
                }
